...
export default {
  ...
  activate(state) {
    ...
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      'provisioning-checker:toggle': () => this.checkEnv()
    }));

    // let cancelButton = atom.views.getView(document.getElementsByClassName('cancel-button')[0]);
    // this.subscriptions.add(atom.tooltips.add(cancelButton, {title: 'This is a tooltip',
    //   placement: 'bottom',
    //   trigger: 'hover'})
    // );
  ...
  checkEnv() {
    ...
    // let key = atom.keymaps.findKeyBindings({command: 'provisioning-checker:toggle'});
    // console.log(key[0].keystrokes);
    this.subscriptions.add(atom.tooltips.add(cancelButton,
      {title: '<span style="font-size: medium; color: #ff0000">hide this panel</span> <br>by click X or toggleme with ',
    ...
    let stdout = function(output) {
      ...
      let stdout = function(output) {
        ...
        var password = ''
        // atom.workspace.addBottomPanel({item: passEditorView, visible: true});
...
function destroyBottomPanels() {
  let myPanels = atom.views.getView(document.getElementsByClassName('provisioning-checker')[0]);
  console.log(myPanels);
  //myPanels.hide();
  // let panels = atom.workspace.getBottomPanels()
  // console.log('panels will destroy = ' + panels);
