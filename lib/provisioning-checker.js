...
export default {
  ...
  passeditor_stdout_cdr1(out) {
    ...
    if (this.myPane == null && info.length > 1) {
      var files = [];
      for (var x in info) {
        let seed = info[x];
        if ( seed != '') {
      ...
      if (this.myPane != null && this.myPane.isActive()) {
        this.myPane.activate();
      } else {
        this.myPane = atom.workspace.getActivePane().splitRight(files.reverse());
      }
    }
  },
