...
export default {
  ...
  rightPane(/* {fileName : TextEditor} */ editors) {
    ...
    if (_rightPane.getItems().length == 0) {
      ...
      lacked = Object.values(editors);
    } else {
      // wanna liveupdate for rightPane
      let nameSet = new Set(Object.keys(editors));
      console.log(nameSet);
      console.log(_rightPane.getItems());
      let panedItems = new Set(_rightPane.getItems().map(function(e) {
        return e.getPath();
      }));
      // 集合演算を持っていない？
      var t = new Set();
    ...
    if ( keynum > 0 ) {
      ...
      for (tE of texts) {
        ...
        if (tE instanceof Promise) {
          ...
          atom.workspace.createItemForURI(tE).then(f);
        }
      }
    }
    return _rightPane;
