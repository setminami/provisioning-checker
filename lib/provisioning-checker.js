...
export default {
  ...
  deactivate() {
    ...
    if (this.humanreadable_files != null) {
      this.humanreadable_files.length = 0;
    }
  ...
  passeditor_stdout_cdr1(out) {
    ...
    if (info[0] == this.SUCCESS_MSG) {
      ...
      for (f of this.OUT_FILES) {
        console.log(f);
        files.push({item: atom.workspace.open(f, {pending: true})});
      }
      this.humanreadable_files = files;

      if (this.rightPane == null) {
        let pane = atom.workspace.getActivePane();
        this.rightPane = pane.splitRight(this.humanreadable_files);
        console.log('>>> rightPane');
        console.log(this.rightPane);
      } else {
        this.rightPane.activate();
      }
    }
  },
  ...
  toggle() {
    ...
    if (this.modalPanel.isVisible()) {
      ...
      if (this.rightPane != null && this.rightPane.isActive()) {
        this.rightPane.destroy();
        this.rightPane = null;
        // rightPaneが開かれているケースではfileが存在する
        this.humanreadable_files.length = 0;
      }
      return;
    }
