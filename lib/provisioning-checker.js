...
export default {
  ...
  activate(state) {
    ...
    this.basePath = atom.packages.resolvePackagePath(this.myName);
    ...
    this.OUT_FILES = filenames.map(function(name) {
      return path.normalize(this.basePath + '/appendix/tmp/' + name);
    });

    // local assertion for promise
    this.getEmitter().setMaxListeners(15);
    let direction = this.myName + '.direction';
  ...
  // targetPanelは'cancel-button' classを必ず持つこと
  ...
  passEditor_keyup_action(event) {
    ...
    switch (event.key) {
      case 'Enter':
        ...
        let outputPath = path.normalize(this.basePath + '/appendix/tmp/');
        // guard
        if (this.checkFiles(p12FileName, provFileName)) { return; }
        let self = this;
        let proc_cdr1 = new BufferedProcess({
          command: path.normalize(this.basePath + '/appendix/ProvisioningChecker.py'),
  ...
  BSDEL_key_handler(keyname, cursorPos, buff) {
    ...
    if (keyname == 'Delete' || keyname == 'Backspace') {
      if (cursorPos < this.password.length && cursorPos > 0) {
  ...
  toggle_stdout_cdr0(output, info) {
    ...
    this.passEditorView.getModel().setText(this.password);
    let self = this;
