...
export default {
  ...
  rightPane(/* {fileName : TextEditor} */ editors) {
    // try not pane, use Dock
    if (this._rightDock == null) {
      // atom.workspace.getActivePane().splitRight();
      this._rightDock = atom.workspace.getRightDock();
      this._rightDock.activate();
    }
    var _rightPane = this._rightDock.getActivePane();
    _rightPane.activate();

    // if (actPane === this.rightPane) {
    //   atom.workspace.activatePreviousPane();
    //   thePane = atom.workspace.getActivePane();
    //   console.log('swap active pane!!');
    // } else {
    //   thePane = actPane;
    //   console.log('hold active pane!!');
    // }
    console.log('_rightPane = ');
    console.log(_rightPane);
    if (_rightPane.getItems().length == 0) {
      console.log('rightPane was empty.');
    } else {
      console.log(this.humanreadable_files);
      // wanna liveupdate for rightPane
      let nameSet = new Set(Object.keys(this.humanreadable_files));
      console.log(nameSet);
      console.log('rightPane Items = ');
      console.log(_rightPane.getItems());
      let panedItems = new Set(_rightPane.getItems());
      console.log(panedItems);
      // for (aI of this.rightPane.getItems()) {
      //   console.log('aI = [' + aI.getPath() + ']');
      //   // not contain
      //   if (nameSet.has(aI.getPath())) {
      //     console.log('donot add!');
      //   } else {
      //     console.log('add!');
      //     this.rightPane.addItem(aI);
      //   }
      // }
    }

    let keynum = Object.keys(editors).length;
    console.log('keynum = ' + keynum);
    if ( keynum > 0 ) {
      let texts = Object.values(editors);
      console.log(texts);
      for (tE of texts) {
        var self = this;
        let item = tE.then(function(editor) {
          console.log(editor);
          _rightPane.addItem(editor);
        });
      }
    }
    return _rightPane;
  ...
  passeditor_stdout_cdr1(out) {
    ...
    if (info[0] == this.SUCCESS_MSG) {
      ...
      this.rightPane(this.humanreadable_files);
    }
  },
  ...
  toggle() {
    ...
    if (this.modalPanel.isVisible()) {
      ...
      if (this._rightDock != null && this._rightDock.isVisible()) { this._rightDock.hide(); }
      return;
    }

    this.modalPanel.show();
    if (this._rightDock != null) { this._rightDock.show()}
