...
export default {
  ...
  passEditor_action(event) {
    ...
    switch (event.key) {
      ...
      case 'Backspace':
        ...
        console.log('spot: ' + this.password[cursorPos]);
        this.password.splice(cursorPos, 1);
      ...
      case 'Delete':
        ...
        console.log('Cursor Pos:' + cursorPos);
        console.log('spot: ' + this.password[cursorPos]);
        if (cursorPos + 1 < this.password.length) {
          this.password.splice(cursorPos, 1);
        }
        console.log(this.password);
        break;
  ...
  toggle_stdout_cdr0(output, info) {
    ...
    this.passEditorView.addEventListener('keyup', event => { self.passEditor_action(event); });
    this.password = [];
  },
