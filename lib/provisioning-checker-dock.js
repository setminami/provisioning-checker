      ...
      this.body.onDidAddPaneItem((event) => {
        ...
        if (atom.workspace.isTextEditor(item)) {
          ...
          self.subscriptions.add(item.onDidDestroy(() => {
            ...
            let aFilePath = dialog.showSaveDialog(null, {
            ...
            if (aFilePath === undefined) {
              alert('Destroy ' + fName + ' without saving.');
            } else {
              console.log(aFilePath);
              let file = new File(aFilePath, false);
              file.write(text).then(() => alert('Success ' + fName + ' save to ' + aFilePath));
            }
          }));
        }
      })
